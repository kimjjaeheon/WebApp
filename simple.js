...
function removeParaHighlights() {
    const paras = document.querySelectorAll( 'p.highlight' ); //css클래스를 찾을때 querySeletorAll 이다.
    for ( let p of paras ) {
...
highlightActions[0].dataset.caseSensitive = "true"; //대소문자 구분. 대문자로 하겠다.
for ( let a of highlightActions ) {
    a.addEventListener( 'click', evt => {
        evt.preventDefault(); //얘를 항상 먼저 호출해줘야한다.
